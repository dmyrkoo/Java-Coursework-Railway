...
Проєкт побудовано за принципами багатошарової **N-Tier архітектури** з використанням архітектурного патерну **Command Query Separation (CQS)**. Це забезпечує слабку зв'язність компонентів (Loose Coupling) та високу тестованість бізнес-логіки.

### Діаграма прецедентів (Use Case)
Відображає основні сценарії взаємодії Диспетчера з системою. Функціонал охоплює повний цикл управління рухомим складом: від перегляду загальної статистики до складних операцій сортування та багатокритеріального пошуку вагонів.

![Діаграма прецедентів](diagrams/Use%20Case%20Diagram.png)

### Діаграма класів (Class Diagram)
Ілюструє доменну модель предметної області та структуру сервісного шару:
* **Доменна модель:** Побудована за принципом «Whole-Part», де абстрактний клас `Vagon` є базовим для конкретних реалізацій (`PasazhyrskyVagon`, `SlyzhbovyVagon`), а клас `Potiag` агрегує їх у єдиний склад.
* **Командний шар (CQS):** Дії інтерфейсу інкапсульовані в окремі команди (наприклад, `AddVagonCommand`, `SortVagonsCommand`), які делегують роботу відповідним сервісам (`SkladService`, `PotiagService`).

![Діаграма класів](diagrams/Class%20Diagram.png)

### Діаграма компонентів (Component Diagram)
Показує фізичний поділ системи на модулі та напрямок потоку управління:
* **UI Layer:** Графічний інтерфейс на базі JavaFX та дизайн-системи AtlantaFX.
* **Layers:** Запити проходять строго через Command Layer -> Service Layer -> Repository Layer.
* **Database:** Персистентність забезпечується вбудованою локальною СУБД SQLite через JDBC.
* **Логування:** `Logback` виступає як наскрізний функціонал (Cross-cutting concern), збираючи метрики та надсилаючи сповіщення про критичні помилки через SMTP.

![Діаграма компонентів](diagrams/Component%20Diagram.png)
...
## ⚙️ Інструкція із запуску та розгортання

### Вимоги до середовища
Перед початком роботи переконайтеся, що на вашій робочій станції встановлені:
* **Java Development Kit (JDK) 17** або вище.
* **Apache Maven** (для збірки проєкту та управління залежностями).
* **Git** (для контролю версій).

### Крок 1. Клонування репозиторію
Склонуйте репозиторій на свій локальний комп'ютер та перейдіть у директорію проєкту:
```bash
git clone [https://github.com/dmyrkoo/Java-Coursework-Railway.git](https://github.com/dmyrkoo/Java-Coursework-Railway.git)
cd Java-Coursework-Railway

```

### Крок 2. Запуск програми

Для компіляції та запуску застосунку виконайте базову команду Maven:
...
*💡 Примітка: Система автоматично завантажить усі необхідні залежності, створить файл бази даних `potiag.db` (якщо його не існує) та заповнить його стартовими даними для демонстрації.*

### Крок 3. Запуск модульних тестів (Опціонально)

Щоб перевірити працездатність бізнес-логіки та запустити всі 161 юніт-тестів (з використанням JUnit 5 та Mockito), виконайте:
